...
async function handleConfigPost(settings, namespace, request, response) {
  ...
  try {
    ...
    await settings.mutate(namespace, ops, typeof body.expectedRevision === 'number' ? body.expectedRevision : undefined)
    sendJson(response, 200, { ok: true })
  } catch (error) {
    const conflict = error instanceof Error && (error.name === 'SettingsConflictError' || error.message.includes('SETTINGS_CONFLICT'))
...
export function mergeChannelSecrets(incomingConfig, currentConfig) {
  ...
  const channels = incomingChannels.map(channel => {
    ...
    for (const key of SECRET_FIELDS[channel.type] ?? []) {
      ...
      const hasProvided = typeof provided === 'string' ? provided.trim() !== '' : provided !== undefined
      if (!hasProvided) {
...
function buildSectionOps(current, next) {
  const ops = []
...
const TEST_SIGNAL = Object.freeze({
  kind: 'loop-completed',
  label: '测试会话',
  summary: '这是一条 multi-channel-notify 测试通知：如果你看到它，说明该渠道链路正常。',
  detail: '',
  severity: 'info',
})
