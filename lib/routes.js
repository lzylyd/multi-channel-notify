 ...
 * - POST /multi-channel-notify/config —— 编辑态全文落库（宿主按渠道 id 合并已存密钥；
 *   expectedRevision 必填，缺失 400），同源 + 主机围栏校验
 * - POST /multi-channel-notify/test   —— 向指定渠道（或全部启用渠道）发送测试消息，返回逐渠道结果
 *
 * 安全：全部端点先过本地主机围栏 isTrustedHost（防 DNS rebinding，配置视图含
 * URL 内嵌凭据明文，GET 同样拦截），写请求再要求同源（Origin/Referer 与 Host 匹配）；
 * 响应不带 CORS 头；GET 无副作用；密钥经 describe({ redactSecrets: true }) 脱敏，
 * secrets sidecar 一并下发供表单渲染 write-only 输入。
 */
...
async function handleConfigPost(settings, namespace, request, response) {
  ...
  // 协议：{ config: 编辑态全文, expectedRevision }（必填）
...
async function handleTestPost({ request, response, dispatcher }) {
  ...
  const results = await Promise.all(
    channels.map(async channel => {
      const outcome = await deliverOnce(dispatcher, channel, message)
      return { id: channel.id, type: channel.type, ok: outcome.ok, ...(outcome.error === undefined ? {} : { error: outcome.error }) }
    }),
  )
...
/** 单渠道一次投递（不重试），返回 { ok, error? }，error 为可直接展示的中文原因。 */
async function deliverOnce(dispatcher, channel, message) {
  try {
    const adapter = ADAPTERS[channel.type]
    if (adapter === undefined) return { ok: false, error: `未知渠道类型：${channel.type}` }
    ...
    dispatcher.hooks.logWarn?.(
      `[multi-channel-notify] 测试失败 ${channel.id}：${reason}`,
    )
