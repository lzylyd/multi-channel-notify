 ...
 * 可选加签（机器人安全设置-加签）。官方三语言示例均为：
 *   string_to_sign = `${timestamp}\n${secret}`
 *   sign = base64(hmac_sha256(key = string_to_sign, message = 空))
 * 注意 key/msg 方向与企业微信相反的钉钉不同；追加到 webhookUrl：
 *   `&timestamp=${timestamp}&sign=${sign}`
 */

import { createHmac } from 'node:crypto'

/** 网络请求默认超时（ms），可用 channel.wecom.timeoutMs 覆盖。 */
const DEFAULT_TIMEOUT_MS = 10_000

/**
 * 计算企业微信加签参数（纯函数）。
 * 黄金向量（ts=1700000000000, secret='mysecret'）：
 *   HRiyltD8KQSp0mWexCskOhD/1F1HDSpwjuNY4CYxZZI=
 * @returns {{ timestamp: number, sign: string }}
 */
export function signatureOf(secret, timestamp = Date.now()) {
  const stringToSign = `${timestamp}\n${secret}`
  // 官方形态：整串作 HMAC key，消息为空
  const hmac = createHmac('sha256', stringToSign).digest('base64')
...
export async function send(channel, message, deps) {
  ...
  const rawTimeout = channel.wecom?.timeoutMs
  const timeoutMs =
