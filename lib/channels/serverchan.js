...
export async function send(channel, message, deps) {
  ...
  if (!response.ok) throw new Error(`serverchan: HTTP ${response.status} ${text.slice(0, 120)}`)
  // 官方 API 失败时也可能返回 HTTP 200 + 非零 errno/code（如配额/密钥问题），
  // 不能只看状态码——把服务端真实原因抛出来（doc.sc3.ft07.com/zh/serverchan3/server/api）
  if (text !== '') {
    try {
      const json = JSON.parse(text)
      const errno = typeof json.errno === 'number' ? json.errno : (typeof json.code === 'number' ? json.code : 0)
      if (errno !== 0) throw new Error(`serverchan: ${json.message ?? json.error ?? text.slice(0, 120)}`)
    } catch (parseError) {
      if (parseError instanceof SyntaxError) return // 非 JSON 响应（空/HTML/纯文本）视为成功
      throw parseError
    }
  }
}
