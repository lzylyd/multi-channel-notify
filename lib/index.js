/**
 * multi-channel-notify —— Cordis Host 入口（社区插件主路径）。
 *
...
/** Cordis 插件名（cordis.patch.yml 的 id/name 保持一致）。 */
export const name = 'multi-channel-notify'

/**
 * settings 命名空间，小写 kebab-case。
 * dsh-settings 0.1.1-rc.1 导出 `settingsNamespace()` 包装器；0.1.2-alpha 已删除该
 * 导出，改为直接接受 kebab 字符串（内部 parseSettingsNamespace 归一化）。为兼容两代，
 * 这里不用具名 import（ESM 具名导入缺失会链接失败），改用命名空间导入 + 运行时兜底。
 * 两个版本都接受字符串，因此始终可解析为 'multi-channel-notify'。
 */
const NAMESPACE = typeof dshSettings.settingsNamespace === 'function'
  ? dshSettings.settingsNamespace('multi-channel-notify')
  : 'multi-channel-notify'

/** WSL 回退探测用的 /proc/version 文本（读取一次；非 Linux 环境为空串）。 */
const PROC_VERSION_TEXT = (() => {
  try {
...
export function apply(ctx, loaderConfig) {
  ...
  ctx.inject(['settings'], settingsCtx => {
    ...
    }), 'multi-channel-notify: settings watch')
  })
  ...
  // 容量护栏：会话标题表有界（Map 保持插入序，超限先删最旧）
  const rememberTitle = (sessionId, title) => {
    if (titles.has(sessionId)) titles.delete(sessionId)
    titles.set(sessionId, title)
    while (titles.size > 500) titles.delete(titles.keys().next().value)
  }
  ...
  ctx.on('session/event', (session, event) => {
    ...
    switch (event.type) {
      ...
      case 'plan/mode': {
        ...
        if (planActive.has(sessionId)) planActive.delete(sessionId)
        planActive.set(sessionId, next)
        while (planActive.size > 500) planActive.delete(planActive.keys().next().value)
  ...
  ctx.on('agent/status', payload => {
    ...
    if (running.has(agent.id)) running.delete(agent.id)
    running.set(agent.id, isRunning)
    while (running.size > 500) running.delete(running.keys().next().value)
  ...
  ctx.effect(() => () => {
    dispatcher.dispose()
    gatekeeper.dispose()
  }, 'multi-channel-notify: dispatcher & gatekeeper disposal')
  ctx.logger.info('[multi-channel-notify] 插件已加载：监听 7 类事件，等待渠道配置')
}
