/**
 * dsh-notify —— dsh-std 标准 FacetModule 入口。
 *
  ...
  const state = observed === undefined
    ? {
        ...
        message:
          ...
          + 'dsh-notify 的事件监听经宿主原生通道在 Cordis 插件行中提供，本标准 facet 保持无副作用。',
      }
...
export function snapshot() {
  return { state: 'degraded', message: '见 activate 发布的 dsh-notify.status' }
}
