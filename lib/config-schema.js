/**
 * multi-channel-notify 配置模型。
 *
 * 同一份 schema 承担两个角色：
 * - Loader config（cordis.patch.yml 中该插件行的 `config:`，成为 settings base 层）；
 * - `ctx.settings.register('multi-channel-notify', Config, { base })` 的命名空间 schema，
...
function channelSchema() {
  return Schema.object({
    ...
    type: Schema.union(['serverchan', 'serverchan-turbo', 'windows-toast', 'wecom', 'webhook']).required().description('渠道类型'),
    ...
    'serverchan-turbo': Schema.object({
      sendKey: Schema.string().role('secret').description('SendKey（sct.ftqq.com/sendkey 页获取，SCT… 开头，推微信）'),
    }),
    ...
    wecom: Schema.object({
      ...
      timeoutMs: Schema.number().default(10_000).description('请求超时毫秒'),
    }),
